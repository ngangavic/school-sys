                    <div class="panel panel-default">
						...
						<form action="input positionsf2t1.php" method="post">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
								...
								<?php
                                 include"connection.php";
                                    $query = mysqli_query($link,"select count(adm) from tbl_meanf2t1 where class='form 2' ");	 
                                    $row=mysqli_fetch_array($query);
                                    $total=$row['count(adm)'];
									$N=$total;

                              $query = mysqli_query($link,"select * from tbl_meanf2t1 where class='form 2' order by mn1 desc");	
                               for($i=1;$i<=$N;$i++){ 
...
<?php } ?>
						...
						<form action="input positionsf2t1.php" method="post">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                              ...
                              $query = mysqli_query($link,"select * from tbl_meanf2t1 where class='form 2' order by mn2 desc");	
                               for($i=1;$i<=$N;$i++){ 
                              while($row=mysqli_fetch_array($query))
                                    {
										$id=$row['id'];
...
<?php } ?>
						...
						<form action="input positionsf2t1.php" method="post">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                              ...
                              $query = mysqli_query($link,"select * from tbl_meanf2t1 where class='form 2' order by mn3 desc");	
                               for($i=1;$i<=$N;$i++){ 
                              while($row=mysqli_fetch_array($query))
                                    {
										$id=$row['id'];
...
<?php } ?>
						...
						<form action="input positionsf2t1.php" method="post">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                              ...
                              $query = mysqli_query($link,"select * from tbl_meanf2t1 where class='form 2' order by mn4 desc");	
                               for($i=1;$i<=$N;$i++){ 
                              while($row=mysqli_fetch_array($query))
                                    {
										$id=$row['id'];
