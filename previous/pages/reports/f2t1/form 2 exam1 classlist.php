...
$pdf->Ln(4);
$pdf->Cell(0,0,'CLASSLIST FORM 2 EXAM 1',0,0,'C');
//$pdf->Ln(8);
$pdf->Ln(0);
...
/*$sqlpos2= mysqli_query($link, "select * from tbl_positonsf2t1 where class='form 2' ORDER BY p1 ASC ");
	while ($row = mysqli_fetch_array($sqlpos2)) {
		...
		$ps3=$row["p3"];
	    $ps4=$row["p4"];*/
$sqlpos2= mysqli_query($link, "SELECT * FROM tbl_positonsf2t1 WHERE class='form 2' ORDER BY p2 ASC ");
	while ($row = mysqli_fetch_array($sqlpos2)) {
		$stdadm=$row['adm'];
		$stdname=$row['name'];
		$ps1=$row["p1"];
